...
1. 最近本地验证强化到达远端后，确认 GitHub Actions CI/security/release workflow
   状态。
2. 在本地 checkout 外配置或确认 GitHub branch protection 与 PyPI/TestPyPI trusted
   publisher setup。
...
检查最近提交的远端 GitHub Actions 结果，并处理 CI/security/release 与本地 `.venv`
验证之间的差异。
