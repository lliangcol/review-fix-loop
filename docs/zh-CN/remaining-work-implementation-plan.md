...
- split CI jobs 存在后配置 branch protection required checks（当前公开
  branch metadata 显示 `main.protected=false`）；
- 为本仓库和 release workflow 配置 PyPI/TestPyPI Trusted Publishing；
- 首次正式 PyPI publish 前运行 TestPyPI 或等价 dry run。

当前远端证据：最近检查的 `Security` workflow 已为 green；PyPI 和 TestPyPI
针对 `review-fix-loop` 的 project JSON 查询均返回 404，因此 release publishing
仍未得到证明。
